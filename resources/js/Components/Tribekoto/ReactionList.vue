	...
	<div>
		...
		<div
			...
			<div class="bg-white p-4 rounded-lg max-w-lg w-full">
				...
				<ul
					v-else
					class="max-w-lg divide-y divide-gray-200 dark:divide-gray-700">
					<li
						v-for="reaction in reactions"
						:key="reaction.user.id"
						class="pb-3 sm:pb-4">
						<div class="flex items-center space-x-4 rtl:space-x-reverse border p-2 rounded-lg">
							<div class="flex-shrink-0">
								<Link :href="route('profile', reaction.user.username)">
									<img
										class="w-8 h-8 rounded-full"
										:src="reaction.user.avatar_url || '/img/default_avatar.png'"
										alt="User avatar" />
								</Link>
							</div>
							...
							<div class="relative group inline-flex items-center">
								<img
									:src="getReactionIcon(reaction.type)"
									:alt="reaction.type"
									class="w-8 h-8 cursor-pointer" />
								<!-- Popover -->
								<span
									class="absolute bottom-full left-1/2 -translate-x-1/2 mb-2 px-2 py-1 text-xs font-medium text-white bg-gray-800 rounded-lg opacity-0 group-hover:opacity-100 transition-opacity">
									{{ reaction.type }}
								</span>
							</div>
						</div>
...
const fetchReactions = async () => {
	...
	try {
		const response = await axiosClient.get(`/post/${props.postId}/reactions`);
