<template>
    <aside class="bg-white dark:bg-slate-950 p-4 shadow my-2 rounded-lg">
        <h2 class="text-2xl font-bold mb-4 dark:text-white">Sponsored Ads</h2>
        <div class="space-y-4">
            ...
            <div v-if="!sponsoredAds.length" class="text-center text-gray-500">
...
const firstSliderAds = computed(() =>
    sponsoredAds.value.filter((ad, index) => index % 2 === 0)
);
const secondSliderAds = computed(() =>
    sponsoredAds.value.filter((ad, index) => index % 2 !== 0)
);
</script>
