	...
	<Link
		...
		<div
			...
			<div class="w-full h-40 overflow-hidden rounded-t-lg">
				<img
					:src="item.attachments[0]?.image_path || 'https://via.placeholder.com/100'"
					:alt="item.name"
			...
			<div class="p-2 flex flex-col space-y-4">
				<div class="flex flex-col w-full space-y-2">
					<h2 class="text-xl font-semibold text-gray-900 dark:text-gray-100 line-clamp-1">
						{{ item.name }}
					</h2>
					<p class="text-lg font-bold text-gray-900 dark:text-gray-200">
						{{ formatPrice(item.price) }}
					</p>
					<p class="text-base text-gray-700 dark:text-gray-300 line-clamp-1">
						{{ item.description }}
					</p>
				</div>
