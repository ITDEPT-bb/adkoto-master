<template>
    <div>
        <div v-if="winner">
            <h2 class="text-green-600 font-bold">
                Winner: {{ winner.name }} with ₱{{ winner.amount }}
            </h2>
        </div>
    ...
    <div class="bg-gray-100 p-4 rounded">
        <p class="text-lg font-bold">⏳ Time Left: {{ timeLeft }}s</p>
    </div>
        ...
        <div class="px-4 mx-auto 2xl:px-0 my-4">
            <div class="lg:grid lg:grid-cols-3 lg:gap-8 xl:gap-10">
                ...
                >
                    ...
                    <!-- <div class="mt-4 sm:items-center sm:gap- sm:flex-col">
                        <p
                            class="text-md font-extrabold text-gray-600 sm:text-md dark:text-white"
                        >
                            Bid Increment:
                    ...
                    <div v-if="item.user.id !== authUser.id">
                        ...
                        <!-- <button
                            @click="placeBid(item.id)"
                            :disabled="isLoadingBid || !isBidValid"
                            :class="[
                                'group my-6 w-full sm:gap-4 sm:items-center justify-center sm:flex sm:my-4 rounded-md p-2',
                                {
                                    'bg-blue-200 text-white cursor-not-allowed':
                                        isLoadingBid || isUserHighestBidder || !isBidValid,
                                    'bg-blue-300 hover:bg-blue-500 transition ease-in-out delay-50 hover:-translate-y-1 hover:scale-105 duration-300 hover:text-white':
                                        !isLoadingBid && !isUserHighestBidder || !isBidValid,
                                },
                            ]"
                        ...
                        <button
                            @click="placeBid(item.id)"
                            :disabled="!isBidValid || isLoadingBid"
                            :class="[
                                'group my-6 w-full sm:gap-4 sm:items-center justify-center sm:flex sm:my-4 rounded-md p-2',
                                {
                                    'bg-blue-200 text-white cursor-not-allowed':
...
function showWinner() {
    if (bids.value.length > 0) {
        ...
        winner.value = `${lastBid.user.name} wins with ₱${lastBid.amount}`;
    } else {
        winner.value = "No bids placed.";
    }
}
