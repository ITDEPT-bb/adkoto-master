<template>
    <div class="grid grid-cols-1 lg:grid-cols-12 gap-4 p-4 h-full">
      ...
      <div class="col-span-1 lg:col-span-3 h-full overflow-y-auto bg-white p-4 rounded-lg shadow-md">
      ...
      <!-- Chat Window Section -->
      <div class="col-span-1 lg:col-span-6 h-full overflow-hidden bg-white rounded-lg shadow-md">
        <div class="flex flex-col h-full">
          <div class="bg-gray-100 px-4 py-2 sticky top-0 z-10">
            <h2 class="text-xl font-bold" v-if="selectedUser">{{ selectedUser.name }} {{ selectedUser.surname }}</h2>
            <div v-else class="text-gray-600">Select a user to start a conversation.</div>
          </div>
          <div
            v-if="conversation"
          ...
          >
            <div
              ...
              :key="message.id"
              :class="{
                'justify-end': message.sender_id !== selectedUser.id,
                'justify-start': message.sender_id === selectedUser.id,
              }"
              class="flex mt-2"
            >
              <div
                :class="{
                  'bg-blue-100 text-right': message.sender_id !== selectedUser.id,
                  'bg-gray-100 text-left': message.sender_id === selectedUser.id,
                }"
                class="p-3 rounded-lg max-w-full mx-2 break-words"
              >
                <p class="text-sm">
                  <strong>{{ message.sender_id !== selectedUser.id ? "You" : selectedUser.name }}:</strong>
                  <br />
                  {{ message.message }}
                  <br />
                  <span class="text-xs text-gray-500">{{ formatMessageTimestamp(message.created_at) }}</span>
                </p>
              </div>
            </div>
          </div>
          <div v-else class="flex-1 flex items-center justify-center text-gray-500">Select a user to start a conversation.</div>
          <div class="mt-4 px-4">
            <div class="relative">
              <textarea
                v-if="conversation"
      ...
      <div class="col-span-1 lg:col-span-3 h-full overflow-y-auto bg-white p-4 rounded-lg shadow-md">
        ...
        <div v-if="selectedUser" class="flex items-center space-x-4">
          ...
          />
          <div>
            <h3 class="text-lg font-semibold">{{ selectedUser.name }} {{ selectedUser.surname }}</h3>
          </div>
        </div>
        <div v-else class="text-gray-500">Select a user to view their information.</div>
      </div>
    </div>
  </template>
  ...
  async function sendMessage() {
    try {
      ...
      const response = await axiosClient.post(`/chat/conversations/${selectedUser.value.id}/messages`, {
